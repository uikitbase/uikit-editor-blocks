...
const PanelSliderInner = ( { attributes } ) => {
  const {
    sliderWidth,
    sliderGap,
    sliderDivider,
  ...
  const slider_height_match_data = []; 
  ...
  if ( panelMatch ) { 
    slider_classes.push( 'uk-grid-match' ); 
  }

  if ( ! sliderWidth ) { 
    slider_height_match_data.push( 'target: .wub-image' ); 
  }
  ...
  if ( slider_height_match_data.length ) { 
    slider_attrs['data-uk-height-match'] = slider_height_match_data.join( '; ' ); 
  }
...
class UikitPanelSliderEdit extends Component {
  render() {
    ...
    if ( slidenavPosition ) {
      if ( slidenavPosition !== 'default' && slidenavPosition !== 'outside' ) {
    ...
    if ( slidenavPosition ) {
      ...
      } else if ( slidenavPosition === 'outside' ) {
        slidenav_left_classes.push( 'uk-position-center-left-out' );
    ...
    if ( slidenavPosition ) {
      if ( slidenavPosition === 'default' ) {
        ...
        slidenav_right_classes.push( 'uk-position-center-right-out' );
